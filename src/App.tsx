...
export default function OGCRequestBuilder() {
  ...
  const [presets, setPresets] = useState<Record<string, unknown>>(() => loadJson(PRESET_KEY, {}));
  ...
  function buildUrl(): string {
    ...
    if (service === "WMS") {
      ...
      const common: Record<string, string> = { SERVICE: "WMS", VERSION: wmsVersion, [useCRS ? "CRS" : "SRS"]: crs };
    ...
    if (service === "WFS") {
      ...
      if (request === "GetFeature") {
        const params: Record<string, string | number> = { SERVICE: "WFS", REQUEST: "GetFeature", VERSION: wfsVersion, [typeKey]: layerList, SRSNAME: crs, COUNT: 100, STARTINDEX: 0, ...extras };
      ...
      if (request === "DescribeFeatureType") {
        const params: Record<string, string> = { SERVICE: "WFS", REQUEST: "DescribeFeatureType", VERSION: wfsVersion, ...extras };
  ...
  async function fetchCatalog() {
    try {
      ...
      if (service === "WMS") {
        ...
        const cols = (json.collections || []).map((c: { id: string; title?: string }) => ({ id: c.id, title: c.title }));
        setCollections(cols);
      }
    } catch (e: unknown) {
  ...
  function loadPreset(name: string) {
    const p = (presets as Record<string, Record<string, unknown>>)[name];
    if (!p) return;
    setBaseUrl((p.baseUrl as string) || "");
    setService((p.service as "WMS" | "WFS" | "OAF") || "WMS");
    setWmsVersion((p.wmsVersion as string) || "1.3.0");
    setWfsVersion((p.wfsVersion as string) || "2.0.0");
    setRequest((p.request as string) || "GetMap");
    setSelectedLayers((p.selectedLayers as string[]) || []);
    setSelectedCollection((p.selectedCollection as string) || "");
    // crs is fixed (3301)
    setBbox((p.bbox as string) || BBOX_PRESETS_3301["Terve Eesti"]);
    setWidth((p.width as number) || 1280);
    setHeight((p.height as number) || 1000);
    setFormat((p.format as string) || "image/png");
    setTransparent(!!(p.transparent));
    setStyles((p.styles as string) || "default");
    setCustomParams((p.customParams as { key: string; value: string }[]) || []);
    setOafLimit((p.oafLimit as number) || 100);
    setOafOffset((p.oafOffset as number) || 0);
    setOafDatetime((p.oafDatetime as string) || "");
    setOafFilter((p.oafFilter as string) || "");
    setWfsOutputFormat((p.wfsOutputFormat as string) || "");
  }
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="border rounded-2xl p-4 border-neutral-300 dark:border-neutral-700">
          ...
          <div className="flex gap-2 mt-2">
            <select className="border rounded-xl px-3 py-2" value={service} onChange={(e) => setService(e.target.value as "WMS" | "WFS" | "OAF")}>
              <option>WMS</option>
              <option>WFS</option>
              <option>OAF</option>
            </select>
        ...
        <div className="flex flex-wrap gap-2 mt-2">
          ...
          {Object.keys(presets).map((k) => (
            <span key={k} className="inline-flex items-center gap-2 px-2 py-1 rounded-full border text-sm">
              ...
              <button title="kustuta" onClick={() => { const next = { ...(presets as Record<string, unknown>) }; delete next[k]; setPresets(next); saveJson(PRESET_KEY, next); }}>✕</button>
            </span>
          ))}
        </div>
      </div>
